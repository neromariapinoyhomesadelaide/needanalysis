...
if marker not in s:
    raise SystemExit('Container marker not found')
...
(function(){
  ...
  function headingMatch(text){
    ...
    return headings.find(h => h.textContent.trim().toLowerCase().includes(text.toLowerCase()));
  }
  ...
  function progressTargets(){
    ...
    const income = headingMatch('Gross Income — Applicant 1') || headingMatch('Gross Income') || document.querySelector('[name="base_income"]')?.closest('.card');
  ...
  function setActive(index){
    ...
    steps.forEach((btn,i)=>{
      btn.classList.toggle('active', i === safe);
      btn.classList.toggle('complete', i < safe);
      if(i === safe) btn.setAttribute('aria-current','step'); else btn.removeAttribute('aria-current');
    });
  ...
  steps.forEach((btn,index)=>btn.addEventListener('click',()=>{
    const target = progressTargets()[btn.dataset.key];
    if(!target) return;
  ...
  function updateFromScroll(){
    ...
    requestAnimationFrame(()=>{
      ...
      const offset = (document.getElementById('formProgressNav')?.offsetHeight || 0) + 40;
      let active = 0;
      steps.forEach((btn,index)=>{
        const el = targets[btn.dataset.key];
        if(el && el.getBoundingClientRect().top <= offset) active = index;
      });
      setActive(active);
      ticking = false;
    });
